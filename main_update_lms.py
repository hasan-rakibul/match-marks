...
def main():
    ...
    lms = pd.read_csv(lms_file, index_col=None)
    ...
    lms_col = lms.columns[lms_col-1] # python is 0-based.
    # for local, I will make student_id as index (so, another one column index is vanished there)
    ...
    for index, row in lms.iterrows():
        student_id = str(row["Student ID"]) # convert to string as local index is string
        if student_id not in local.index:
            logging.debug(f"Student ID {student_id} is not in local file. Skipped.")
            continue
        # convert marks to float
        try:
            local_mark = float(local.loc[student_id, local_col])
        except:
            logging.info(Back.RED + "Student ID " + student_id + " has invalid mark formatting in local file: " + local.loc[student_id, local_col] + ". Skipped." + Back.RESET)
            continue
        ...
        if math.isnan(local_mark):
            logging.debug(f"Student ID {student_id} has probable empty mark in local file: {local_mark}. Skipped.")
            continue
        ...
        if type(lms_mark) == str:
            if lms_mark == 'In Progress' or lms_mark == 'Needs Marking': # Those who submitted in LMS
                logging.debug(f"Student ID {student_id} has invalid marking in LMS file: {lms_mark}. Considered as 0.")
                lms_mark = 0.0
        ...
        if math.isnan(lms_mark):
            logging.debug(f"Student ID {student_id} has empty mark in LMS file: {lms_mark}. Considered as 0.")
            lms_mark = 0.0

        if lms_mark < local_mark:
            lastname = lms.loc[index, 'Last Name']
            firstname = lms.loc[index, 'First Name']
            lms.loc[index, lms_col] = local_mark
            logging.info(f"{lastname}, {firstname} ({student_id}): {lms_mark} --> {local_mark}. Updated.")
        elif lms_mark > local_mark:
            logging.info(f"Student ID {student_id} has higher mark in LMS file: {lms_mark} > {local_mark}. Skipped.")
